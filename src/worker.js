import { ReminderScheduler, reminderClientName } from './reminder-scheduler.js';
  ...
  properties: {
    operations: {
      ...
      items: {
        type: "object",
        properties: {
          intent: { type: "string", enum: ["add", "edit", "delete", "clear", "query_availability"] },
          activity: { type: "string", maxLength: 120 },
          replacement_activity: { type: ["string", "null"], maxLength: 120 },
          ...
          duration_minutes: { type: ["number", "null"], minimum: 5, maximum: 720 }
        },
        required: ["intent", "activity", "replacement_activity", "matched_activity_id", "timing", "confidence", "clarification", "duration_minutes"],
        additionalProperties: false
      }
    }
  },
...
5. Use only intents add, edit, delete, clear, query_availability.
5a. Use query_availability when the user asks when they are free, what open slots exist, whether an activity can fit, or asks for available time between anchors/times. For this intent, activity may be a requested activity name or the word availability, matched_activity_id must be null, and duration_minutes is the requested duration when stated (otherwise null).
...
13. between stores the first anchor in anchor and the second in anchor2. Do not invent midpoint times. For query_availability, anchors may also be explicit 24-hour HH:MM strings, e.g. "between 2 and 5 PM" => anchor="14:00", anchor2="17:00".
14. For edits, activity is ALWAYS the existing activity to locate in the supplied schedule. replacement_activity is the new activity name only when the user wants to replace/rename it. Examples: "move gym from 6 to 8" => activity="Gym", replacement_activity=null, timing exact_time 20:00; "replace gym with swimming" => activity="Gym", replacement_activity="Swimming", timing untimed; "change my meeting at 2 to swimming at 3" => activity="Meeting", replacement_activity="Swimming", timing exact_time 15:00.
14a. A time mentioned before the word "to" can identify the existing target (for example "meeting at 2"); do NOT return that old time as the new timing. The timing object describes the NEW requested placement only.
14b. An edit is complete when it has either a new timing, a replacement_activity, or both. Do not ask for a time when the user only wants to replace/rename the activity in the same slot.
...
17. Availability questions are read-only. Never convert them into add/edit/delete operations. Examples: "When am I free between Dhuhr and Asr?" => query_availability with timing.type="between", anchor="Zohr", anchor2="Asr". "Can I fit a 45 minute walk before Maghrib?" => query_availability, activity="walk", duration_minutes=45, timing.type="before_prayer", anchor="Maghrib".
18. Return only the required JSON structure. No prose.`;
...
function sanitizeGroqInterpretation(value) {
  ...
  const allowedIntent = new Set(["add", "edit", "delete", "clear", "query_availability"]);
  ...
  for (const raw of value.operations.slice(0, 20)) {
    ...
    if (!intent || (!["clear","query_availability"].includes(intent) && !activity)) continue;
    ...
    operations.push({
      intent,
      activity: intent === "clear" ? (activity || "schedule") : (intent === "query_availability" ? (activity || "availability") : activity),
      replacement_activity: intent === "edit" ? (cleanLabel(raw?.replacement_activity, 120) || null) : null,
      matched_activity_id: ["add","clear","query_availability"].includes(intent) ? null : (cleanLabel(raw?.matched_activity_id, 120) || null),
      timing,
      confidence: ["high", "medium", "low"].includes(raw?.confidence) ? raw.confidence : "medium",
      clarification: cleanLabel(raw?.clarification, 180) || null,
      duration_minutes: Number.isFinite(Number(raw?.duration_minutes)) ? Math.max(5, Math.min(720, Number(raw.duration_minutes))) : null
    });
  }
  return { operations };
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/reminders/config") {
      if (request.method !== "GET") return new Response(null, { status:405, headers:{ allow:"GET" } });
      const configured = !!(env.VAPID_PUBLIC_KEY && env.VAPID_PRIVATE_KEY && env.REMINDER_SCHEDULER);
      return json({
        configured,
        vapidPublicKey: env.VAPID_PUBLIC_KEY || null,
        message: configured ? null : "Reminder push needs VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY configured in Cloudflare."
      });
    }

    if (url.pathname.startsWith("/api/reminders/")) {
      if (!env.REMINDER_SCHEDULER) return json({ error:"Reminder scheduler binding is not configured." }, 503);
      let body = {};
      if (request.method !== "GET") {
        try { body = await request.clone().json(); } catch (_) { return json({ error:"Invalid JSON." }, 400); }
      }
      const clientName = reminderClientName(body);
      if (!clientName) return json({ error:"Invalid reminder client." }, 400);
      const id = env.REMINDER_SCHEDULER.idFromName(clientName);
